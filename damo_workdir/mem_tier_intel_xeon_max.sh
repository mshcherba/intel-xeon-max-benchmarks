...
"$damo_bin" record \
	` # promote hot memory from node 0 to node 2 (HBM) ` \
	--numa_node 0 --monitoring_intervals_goal 4% 3 5ms 10s \
		--damos_action migrate_hot 2 --damos_access_rate 5% max \
		--damos_apply_interval 1s \
		` # up to 200 MB per second ` \
		--damos_quota_interval 1s --damos_quota_space 200MB \
		` # aiming node 2 has at least 99.7% memory utilization ` \
		--damos_quota_goal node_mem_used_bp 99.7% 2 \
		--damos_filter allow young \
	` # promote hot memory from node 1 to node 3 (HBM) ` \
	--numa_node 1 --monitoring_intervals_goal 4% 3 5ms 10s \
		--damos_action migrate_hot 3 --damos_access_rate 5% max \
		--damos_apply_interval 1s \
		` # up to 200 MB per second ` \
		--damos_quota_interval 1s --damos_quota_space 200MB \
		` # aiming node 3 has at least 99.7% memory utilization ` \
		--damos_quota_goal node_mem_used_bp 99.7% 3 \
		--damos_filter allow young \
	` # demote cold memory from node 2 (HBM) to node 0 ` \
	--numa_node 2 --monitoring_intervals_goal 4% 3 5ms 10s \
		--damos_action migrate_cold 0 --damos_access_rate 0% 0% \
		--damos_apply_interval 1s \
		` # up to 200 MB per second ` \
		--damos_quota_interval 1s --damos_quota_space 200MB \
		` # aiming node 2 has at least 0.5% free space ` \
		--damos_quota_goal node_mem_free_bp 0.5% 2 \
		--damos_filter reject young \
	` # demote cold memory from node 3 (HBM) to node 1 ` \
	--numa_node 3 --monitoring_intervals_goal 4% 3 5ms 10s \
		--damos_action migrate_cold 1 --damos_access_rate 0% 0% \
		--damos_apply_interval 1s \
		` # up to 200 MB per second ` \
		--damos_quota_interval 1s --damos_quota_space 200MB \
		` # aiming node 3 has at least 0.5% free space ` \
		--damos_quota_goal node_mem_free_bp 0.5% 3 \
		--damos_filter reject young \
	--damos_nr_quota_goals 1 1 1 1 --damos_nr_filters 1 1 1 1 \
	--nr_targets 1 1 1 1 --nr_schemes 1 1 1 1 --nr_ctxs 1 1 1 1
